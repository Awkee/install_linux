...
add_service() {
    ...
    do
        ...
        fn="/usr/lib/systemd/system/${service_name}.service"
        if [ -f "$fn" ] ; then
            read -p "服务文件[${fn}]已经存在！输入个新服务名(例如:ss2go01,或回车退出)?" your_answer
            if [ "${your_answer}" = "" ] ; then
                exit 0
            fi
            service_name="${your_answer}"
...
add_port() {
    echo "开始添加 iptables 防火墙配置"
    ss_port="${1:-12345}"
    if netstat -utlnp | grep "${ss_port}" | grep -v grep > /dev/null ; then 
        LOG "${ss_port}" "端口监听中"
    else
    ...
    # 限制每个客户端的HTTPS服务连接数
    iptables -I INPUT -p tcp --syn --dport ${ss_port} -m connlimit --connlimit-above ${maxlink} -j REJECT
    iptables -I INPUT -p tcp --syn --dport ${ss_port} -m connlimit --connlimit-upto ${maxlink}  -j ACCEPT
}
...
del_port() {
    ...
    iptables -D INPUT -m state --state NEW -m udp -p udp --dport ${ss_port} -j ACCEPT
    # 限制每个客户端的HTTPS服务连接数
...
select_cipher(){
    ...
    read -p "$(echo -e 选择加密方法(默认:1)):" your_answer
    if [ "$your_answer" = "" ] ; then
        LOG "您选择使用默认值:1"
        your_answer="1"
...
select_port() {
    ...
    do
        ...
        if netstat -tanp|grep LISTEN |grep ":${your_answer}" ; then
...
del_server(){
    ...
    if grep "$service_name" ${conf_file} >/dev/null ; then
        ...
        if systemctl is-active ${service_name} ; then
            ...
            sport=`systemctl cat ssserver.service| sed 's/'\''//g' | awk -F: '/^ExecStart/{print $NF }'`
            del_port ${sport}
        fi
...
info() {
    service_name=${1:-"ssserver"}
    if [ -f "/usr/lib/systemd/system/${service_name}.service" ] ; then
        uri_str=`get_uri /usr/lib/systemd/system/${service_name}.service`
    else
    ...
    share_uri="ss://`echo -e "${params}\c" |base64 -w0`"
...
Usage:
    ...
    `basename $0` status [service]              # 查看SS链接URI信息(默认service名为ssserver或ssclient)
END
}
